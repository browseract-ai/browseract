"""
Description:
Resume a paused agent task and continue execution from where it left off. 
This operation is only available for tasks that are currently in a paused state - stopped or completed tasks cannot be resumed.

Documentation:
https://www.browseract.com/reception/integrations/api-agent

curl -X PUT 'https://api.browseract.com/v2/agent/resume-task?task_id=1234567890' -H 'Authorization: Bearer app-abcdefghijklmn'
"""
...
def main():
    ...
    try:
        ...
        api_url = f"https://api.browseract.com/v2/agent/resume-task?task_id={task_id}"
